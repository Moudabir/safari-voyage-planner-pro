...
create policy if not exists "Users can view their own trip shares"
  on public.trip_shares for select
  to authenticated
  using (auth.uid() = created_by);

create policy if not exists "Users can create trip shares for their trips"
...
create policy if not exists "Users can update their own trip shares"
  on public.trip_shares for update
  to authenticated
  using (auth.uid() = created_by);

create policy if not exists "Users can delete their own trip shares"
